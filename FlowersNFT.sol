...
contract Flower is ERC721, ERC721URIStorage, AutomationCompatibleInterface {
    ...
    // Calls the Counter struct with all its funcs from the Counters library.
    // So we dont implement raw logics here.
    // Just call the func of that library that handles that logic
   ...
   // Metadata information for each stage of the NFT on IPFS.
    ...
    function safeMint(address to) public {
        uint256 tokenId = tokenIdCounter.current(); // Sets newly minted NFT tokenId to last saved tokenId value
        tokenIdCounter.increment(); // Increases and stores the tokenID for next NFT mint
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, IpfsUri[0]); // Allocates
    }
    ...
    // To view updated NFT on opensea testnet
    // Go to testnets.opensea.io
    // Input NFT contract address in search bar
    // Click on 3 dots to right of page (...) and click 'refresh metadata'.
    // Right click on page, click >> after 'Source"
    // Then 'Applications'
    // Then 'Clear sites data'
    // Now refresh page.
